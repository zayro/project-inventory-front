...
export class SupplierComponent extends GeneralService implements AfterViewInit, OnInit, OnDestroy {
  ...
  RedirectEdit(info: any): void {
    // this.message = info.id + ' - ' + info.firstName;
    //console.log(info);
    this.router.navigate(['supplier/edit/' + info.id]);
  }
  ...
  getData() {
    ...
    this.table_datable = $('#TableDbProveedor').DataTable({
      ...
      pageLength: 5,
      ajax: {
        url: `${this.environments.api}/${this.environments.prefix}/${this.environments.db}/all/view_proveedor`,
        type: 'GET',
